...
public class SiTsaTimeStampRFC3161 {
    ...
    public void stamp() {
        ...
        try {
            ...
            LOG.log("Status = " + response.getStatusString());
            ...
            LOG.log("Timestamp: " + Base64.getEncoder().encodeToString(response.getEncoded()));
            LOG.log("TSA: " + response.getTimeStampToken().getTimeStampInfo().getTsa());
            LOG.log("Serial number:" + response.getTimeStampToken().getTimeStampInfo().getSerialNumber());
            LOG.log("Policy: " + response.getTimeStampToken().getTimeStampInfo().getPolicy());
        } catch (Exception e) {
            LOG.logError("Error occured while generating timestamp: ", e);
        }
    }
